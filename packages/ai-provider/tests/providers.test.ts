...
describe('activeProvider', () => {
  it('honors a configured BYOK provider and falls back to claude-code otherwise', () => {
    const settings = defaultAiSettings()
    expect(activeProvider(settings)).toBe('claude-code')

    settings.provider = 'kimi'
    expect(activeProvider(settings)).toBe('claude-code') // no key yet
  ...
  it('requires a base URL for providers that declare needsBaseUrl', () => {
    ...
    expect(activeProvider(settings)).toBe('claude-code') // custom's default model is empty
  ...
  it('treats whitespace-only keys, URLs, and models as unconfigured', () => {
    ...
    expect(activeProvider(custom)).toBe('claude-code')
  })

  it('falls back to claude-code for unknown ids from a hand-edited settings file', () => {
    const settings = defaultAiSettings()
