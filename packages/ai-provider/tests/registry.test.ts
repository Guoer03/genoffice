...
describe('provider registry', () => {
  ...
  it('authenticates via the local CLI for codex/claude-code and an api key otherwise', () => {
    expect(AI_PROVIDER_ADAPTERS.codex.capabilities.auth).toBe('codex-chatgpt')
    expect(AI_PROVIDER_ADAPTERS['claude-code'].capabilities.auth).toBe('claude-subscription')
    for (const [id, adapter] of Object.entries(AI_PROVIDER_ADAPTERS)) {
      if (id === 'codex' || id === 'claude-code') continue
