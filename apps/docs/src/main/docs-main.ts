...
import { listClaudeCodeModels, shutdownClaudeCodeAppServers } from '@genoffice/ai-provider/claude-code-app-server'
...
const tMain = createI18n({
  zh: {
    ...
    errNotImage: '不是支持的图片类型',    errNoApiKey: '未配置 {provider} 的 API Key',
  ...
  en: {
    ...
    errNotImage: 'not a supported image type',    errNoApiKey: 'No API key configured for {provider}',
  ...
  ja: {
    ...
    errNotImage: 'サポートされていない画像形式です',    errNoApiKey: '{provider} の API キーが設定されていません',
  ...
  ko: {
    ...
    errNotImage: '지원되지 않는 이미지 형식입니다',    errNoApiKey: '{provider}의 API 키가 설정되지 않았습니다',
  ...
  fr: {
    ...
    errNotImage: "type d'image non pris en charge",    errNoApiKey: 'Aucune clé API configurée pour {provider}',
  ...
  de: {
    ...
    errNotImage: 'kein unterstütztes Bildformat',    errNoApiKey: 'Kein API-Schlüssel für {provider} konfiguriert',
  ...
  es: {
    ...
    errNotImage: 'no es un tipo de imagen compatible',    errNoApiKey: 'No hay clave de API configurada para {provider}',
    errAiBusy:
  ...
  th: {
    ...
    errNotImage: 'ไม่ใช่ชนิดรูปภาพที่รองรับ',    errNoApiKey: 'ยังไม่ได้ตั้งค่า API Key ของ {provider}',
  ...
  id: {
    ...
    errNotImage: 'bukan jenis gambar yang didukung',    errNoApiKey: 'API Key untuk {provider} belum dikonfigurasi',
  ...
  ru: {
    ...
    errNotImage: 'неподдерживаемый тип изображения',    errNoApiKey: 'API-ключ для {provider} не настроен',
  ...
  ar: {
    ...
    errNotImage: 'ليس نوع صورة مدعومًا',    errNoApiKey: 'لم يتم تكوين مفتاح API لـ {provider}',
  ...
  pt: {
    ...
    errNotImage: 'não é um tipo de imagem suportado',    errNoApiKey: 'Nenhuma chave de API configurada para {provider}',
  ...
  it: {
    ...
    errNotImage: 'tipo di immagine non supportato',    errNoApiKey: 'Nessuna chiave API configurata per {provider}',
  ...
  pl: {
    ...
    errNotImage: 'nieobsługiwany typ obrazu',    errNoApiKey: 'Nie skonfigurowano klucza API dla {provider}',
  ...
  cs: {
    ...
    errNotImage: 'nepodporovaný typ obrázku',    errNoApiKey: 'Pro {provider} není nakonfigurován žádný klíč API',
  ...
  nl: {
    ...
    errNotImage: 'geen ondersteund afbeeldingstype',    errNoApiKey: 'Geen API-sleutel geconfigureerd voor {provider}',
  ...
  ms: {
    ...
    errNotImage: 'bukan jenis imej yang disokong',    errNoApiKey: 'Kunci API untuk {provider} belum dikonfigurasikan',
  ...
  he: {
    ...
    errNotImage: 'סוג תמונה שאינו נתמך',    errNoApiKey: 'לא הוגדר מפתח API עבור {provider}',
  ...
  hi: {
    ...
    errNotImage: 'समर्थित छवि प्रकार नहीं है',    errNoApiKey: '{provider} के लिए कोई API कुंजी कॉन्फ़िगर नहीं है',
  ...
  'zh-TW': {
    ...
    errNotImage: '不是支援的圖片類型',    errNoApiKey: '未設定 {provider} 的 API Key',
...
export function registerAiIpc(): void {
  ...
  ipcMain.handle('ai:claude-code-models', async (_event, cliPath: unknown) => {
    return listClaudeCodeModels(typeof cliPath === 'string' ? cliPath : undefined)
  })
  ...
  ipcMain.handle('ai:search-test', (_event, input: unknown) => {
    ...
    if (!provider) return { ok: false, error: 'No search provider configured' }
  ...
  ipcMain.handle('ai:media-test', (_event, input: unknown) => {
    ...
    if (!provider) return { ok: false, error: 'No media provider configured' }
