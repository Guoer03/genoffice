...
const tMain = createI18n({
  ...
  ko: {
    ...
    errNotImage: '지원되는 이미지 형식이 아닙니다',    errNoApiKey: '{provider}의 API 키가 설정되지 않았습니다',
  ...
  de: {
    ...
    errNotImage: 'kein unterstützter Bildtyp',    errNoApiKey: 'Kein API-Schlüssel für {provider} konfiguriert',
  ...
  ar: {
    ...
    errNotImage: 'نوع صورة غير مدعوم',    errNoApiKey: 'لم يتم تكوين مفتاح API لـ {provider}',
...
async function applyMainProcessProxy(): Promise<void> {
  ...
  try {
    await app.whenReady()
    // PAC/rule proxies answer per-host: probe a representative LLM endpoint
