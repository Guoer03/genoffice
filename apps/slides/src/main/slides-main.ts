...
export function registerSlidesIpc(): void {
  ...
  // ── Cloud single-page generation was a Genspark-only feature (gsk slide_generate)
  // and has been removed. The status channel stays so renderer UI degrades gracefully
  // (cloud gen hidden) instead of calling a missing handler.
  ...
  // primitives — no HTML intermediate. Returns the same marker kind as the removed
  // cloud path, so
...
async function applyMainProcessProxy(): Promise<void> {
  ...
  try {
    await app.whenReady()
    // PAC/rule proxies answer per-host: probe a representative endpoint the
    // main-process fetch hits (BYOK providers, search backends)
